...
function TrangBuildPC() {
  ...
  useEffect(() => {
    ...
    if (cpu && main) {
      ...
      const cpuS = cpu.thongSo
        ?.match(regexSocket)?.[0]
        ?.replace(/\s|Socket/gi, "");
      const mainS = main.thongSo
        ?.match(regexSocket)?.[0]
        ?.replace(/\s|Socket/gi, "");
      if (cpuS && mainS && cpuS !== mainS) {
        errors.push(
          `❌ Lỗi Socket: CPU (${cpuS}) không khớp với Mainboard (${mainS}).`,
        );
      }
    }
    ...
    if (main && ram) {
      ...
      if (mRam && rRam && mRam !== rRam) {
        errors.push(
          `❌ Lỗi RAM: Mainboard hỗ trợ ${mRam} nhưng RAM là ${rRam}.`,
        );
      }
    }
  ...
  const handleAction = (type) => {
    if (!userStorage) {
      if (
        window.confirm("Bạn cần đăng nhập để tiếp tục. Đi đến trang đăng nhập?")
      ) {
        navigate("/login");
      }
      return;
    ...
    if (type === "ADD_ALL") {
      ...
      alert(
        `Tổng tiền: ${tongTien.toLocaleString()} đ. Đang chuyển hướng thanh toán...`,
      );
    }
  };
            ...
            <div style={styles.sidebar}>
              ...
              <div style={styles.buildItems}>
                {danhSachChon.length === 0 && (
                  <p style={styles.empty}>Chưa có linh kiện nào</p>
                )}
                {danhSachChon.map((item) => (
                  <div key={item._id} style={styles.itemRow}>
                    ...
                    <button
                      onClick={() =>
                        setDanhSachChon(
                          danhSachChon.filter((i) => i._id !== item._id),
                        )
                      }
                      style={styles.btnDel}
                    >
